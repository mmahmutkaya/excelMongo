/**
 * SOZLESMELER (Contracts) Collection - Integration Test Guide
 *
 * This file demonstrates how to test the sozlesmeler API endpoints
...
const testCreateContract = async () => {
  ...
  const projectId = "YOUR_PROJE_ID_HERE" // Get from existing proje

  try {
    const response = await fetch('http://localhost:4000/api/sozlesmeler', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        paraBirimi: "TRL",
        odemeProgrami: "3 taksit",
        durum: "Aktif",
        notlar: "Test sözleşmesi"
      })
    })
...
const testGetContractsByProject = async (projectId) => {
  ...
  try {
    const response = await fetch(`http://localhost:4000/api/sozlesmeler/byproje/${projectId}`, {
      method: 'GET',
      headers: {
        'email': email,
        'token': token
...
const testGetContract = async (contractId) => {
  ...
  try {
    const response = await fetch(`http://localhost:4000/api/sozlesmeler/${contractId}`, {
      method: 'GET',
      headers: {
        'email': email,
        'token': token
  ...
  try {
    const response = await fetch(`http://localhost:4000/api/sozlesmeler/${contractId}`, {
      ...
      body: JSON.stringify({
        ...
        odemeProgrami: "4 taksit - geliştirilmiş"
      })
    })
...
const testUpdateStatus = async (contractId) => {
  ...
  try {
    const response = await fetch(`http://localhost:4000/api/sozlesmeler/${contractId}/status`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        'email': email,
...
const testAddSignatory = async (contractId) => {
  ...
  try {
    const response = await fetch(`http://localhost:4000/api/sozlesmeler/${contractId}/imzaci`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'email': email,
    ...
    // const imzaciId = data.imzacilar[data.imzacilar.length - 1]._id
 ...
 *
 *    const projectId = "YOUR_PROJE_ID" // Replace with actual ID
 *
 *    // Create contract
 ...
 * 2. Create Contract:
 *    - Method: POST
 *    - URL: http://localhost:4000/api/sozlesmeler
 *    - Headers:
 *      - Content-Type: application/json
 *      - email: your@email.com
 ...
 * 3. Get Contracts:
 *    - Method: GET
 *    - URL: http://localhost:4000/api/sozlesmeler/byproje/[PROJECT_ID]
 *    - Headers:
 *      - email: your@email.com
 *      - token: [JWT_TOKEN]
 *
 ...
 * curl -X GET http://localhost:4000/api/sozlesmeler/byproje/[PROJECT_ID] \
 *   -H "email: your@email.com" \
 *   -H "token: [JWT_TOKEN]"
 *
 * Update Contract:
 * curl -X PATCH http://localhost:4000/api/sozlesmeler/[CONTRACT_ID] \
 *   -H "Content-Type: application/json" \
 *   -H "email: your@email.com" \
 *   -H "token: [JWT_TOKEN]" \
 *   -d '{"sozlesmeAmount": 600000}'
 *
 * Update Status:
 * curl -X PATCH http://localhost:4000/api/sozlesmeler/[CONTRACT_ID]/status \
 *   -H "Content-Type: application/json" \
 *   -H "email: your@email.com" \
 *   -H "token: [JWT_TOKEN]" \
 *   -d '{"durum": "Tamamlandı"}'
 *
 * Add Signatory:
 * curl -X POST http://localhost:4000/api/sozlesmeler/[CONTRACT_ID]/imzaci \
 *   -H "Content-Type: application/json" \
 *   -H "email: your@email.com" \
 *   -H "token: [JWT_TOKEN]" \
 *   -d '{
 ...
 *
 * Delete Contract:
 * curl -X DELETE http://localhost:4000/api/sozlesmeler/[CONTRACT_ID] \
 *   -H "email: your@email.com" \
 *   -H "token: [JWT_TOKEN]"
 */
